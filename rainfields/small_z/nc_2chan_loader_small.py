...
def HimfieldsDataset(fnames, mult, batch_size=2):
    ...
    ds = ds.interleave(lambda fname: tf.data.Dataset.from_generator(gen(), (tf.float32, tf.float32, tf.float32), (tf.TensorShape([1024, 1024, 2]), tf.TensorShape([256,256,3]), tf.TensorShape([1024, 1024, 1])), args=(fname, mult)), cycle_length=len(fnames), block_length=1, num_parallel_calls=None)
