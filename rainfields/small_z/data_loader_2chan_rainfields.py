    ...
    def __call__(self, fname, insert=None, jitter=False):
        ...
        for t in dsg.time:
            ...
            if insert=="z":
                ...
                gpm = dsgpm['PrecCal'].sel(time=t, method='nearest').data[1:, 101:]
            ...
            if insert=="z":
                ...
                yield (np.stack((b8,b14), axis=-1), gpm[:,:,None], prec[:, :, None])
            else:
...
def HimfieldsDataset(fnames, insert, batch_size=2):

    ds = tf.data.Dataset.from_tensor_slices(fnames)
    if insert=="z":
        ds = ds.interleave(lambda fname: tf.data.Dataset.from_generator(gen(), (tf.float32, tf.float32, tf.float32), (tf.TensorShape([1024, 1024, 2]), tf.TensorShape([256,256,3]), tf.TensorShape([1024, 1024, 1])), args=(fname, insert)), cycle_length=len(fnames), block_length=1, num_parallel_calls=None)
    elif insert=="gpm":
        ds = ds.interleave(lambda fname: tf.data.Dataset.from_generator(gen(), (tf.float32, tf.float32, tf.float32), (tf.TensorShape([1024, 1024, 2]), tf.TensorShape([512,512,1]), tf.TensorShape([1024, 1024, 1])), args=(fname, insert)), cycle_length=len(fnames), block_length=1, num_parallel_calls=None)
    else:
        ds = ds.interleave(lambda fname: tf.data.Dataset.from_generator(gen(), (tf.float32, tf.float32), (tf.TensorShape([1024, 1024, 2]), tf.TensorShape([1024, 1024, 1])), args=(fname, insert)), cycle_length=len(fnames), block_length=1, num_parallel_calls=None)
