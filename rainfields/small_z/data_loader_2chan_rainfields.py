...
def HimfieldsDataset(fnames, z, batch_size=2):
    ...
    if z:
        ds = ds.interleave(lambda fname: tf.data.Dataset.from_generator(gen(), (tf.float32, tf.float32, tf.float32), (tf.TensorShape([1024, 1024, 2]), tf.TensorShape([256,256,3]), tf.TensorShape([1024, 1024, 1])), args=(fname, z)), cycle_length=len(fnames), block_length=1, num_parallel_calls=None)
    else:
        ds = ds.interleave(lambda fname: tf.data.Dataset.from_generator(gen(), (tf.float32, tf.float32), (tf.TensorShape([1024, 1024, 2]), tf.TensorShape([1024, 1024, 1])), args=(fname, z)), cycle_length=len(fnames), block_length=1, num_parallel_calls=None)
