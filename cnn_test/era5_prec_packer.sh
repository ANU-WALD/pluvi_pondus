...
while [ "$DATE" != "$END" ]; do
	...
	cdo remapnn,outgrid.txt /g/data/fj4/scratch/infilep.nc /g/data/fj4/scratch/eu_tmp_$(date +%Y%m%d%H%M -d "$DATE").nc
	rm /g/data/fj4/scratch/infilep.nc
	#cdo info /g/data/fj4/scratch/eu_tmp_$(date +%Y%m%d%H%M -d "$DATE").nc
