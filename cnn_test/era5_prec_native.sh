...
while [ "$DATE" != "$END" ]; do
	...
	cdo sellonlatbox,-50.0,40.0,75.0,15.0 /g/data/fj4/scratch/tmp1.nc /g/data/fj4/scratch/eu_tmp_$(date +%Y%m%d%H%M -d "$DATE").nc
	rm /g/data/fj4/scratch/tmp?.nc
	DATE=$(date -d "$DATE + 1 month" -u)
done

cdo -b f32 mergetime /g/data/fj4/scratch/eu_tmp_*.nc /g/data/fj4/scratch/EU_NATIVE_TP_ERA5.nc
