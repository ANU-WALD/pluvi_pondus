...
while [ "$DATE" != "$END" ]; do
	for BAND in {1..16}; do
		...
		if [ -f $FILE ]; then
			gdalwarp -of netCDF -r bilinear -t_srs '+proj=longlat +datum=WGS84 +no_defs' -te 90.0 -60.0 18.0 -0.0 -tr 0.25 -0.25 $FILE B$BAND.nc
			ncrename -v Band1,B$BAND B$BAND.nc
		fi
	done
	cdo merge B1.nc B2.nc B3.nc B4.nc B5.nc B6.nc B7.nc B8.nc B9.nc B10.nc B11.nc B12.nc B13.nc B14.nc B15.nc B16.nc HIM8_$(date +%Y%m%d%H%M -d "$DATE")_AU.nc
	rm B*.nc
