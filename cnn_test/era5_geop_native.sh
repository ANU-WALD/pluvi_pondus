...
while [ "$DATE" != "$END" ]; do
	...
	cdo sellonlatbox,-50.0,40.0,75.0,15.0 /g/data/fj4/scratch/tmpz1.nc /g/data/fj4/scratch/eu_tmpz_$(date +%Y%m%d%H%M -d "$DATE").nc
	rm /g/data/fj4/scratch/tmpz?.nc
	DATE=$(date -ud "$DATE + 1 month")
done

cdo -b f32 mergetime /g/data/fj4/scratch/eu_tmpz_*.nc /g/data/fj4/scratch/EU_NATIVE_Z_ERA5.nc
