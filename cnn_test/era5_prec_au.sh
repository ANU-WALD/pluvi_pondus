...
while [ "$DATE" != "$END" ]; do
	...
	cdo sellonlatbox,90.0,180.0,0.0,-60.0 /g/data/fj4/scratch/tmp1.nc /g/data/fj4/scratch/eu_tmp_$(date +%Y%m%d%H%M -d "$DATE").nc
	rm /g/data/fj4/scratch/tmp?.nc
	DATE=$(date -d "$DATE + 1 month" -u)
done

cdo -b f32 mergetime /g/data/fj4/scratch/eu_tmp_*.nc /g/data/fj4/scratch/AU_NATIVE_TP_ERA5.nc
